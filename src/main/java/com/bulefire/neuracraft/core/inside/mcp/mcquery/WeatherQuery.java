        ...
        if (params.get("level") instanceof String level) {
            ...
            return switch (level) {
                case "overworld" -> MCPMessage.responseBuilder()
                                              .id(request.getId())
                                              .result(MCPResponse.Result.of("overworld 当前天气: "+ getWeatherAndRainfall(Objects.requireNonNull(server.getLevel(Level.OVERWORLD)))))
                                              .build();
                case "nether" -> MCPMessage.responseBuilder()
                                           .id(request.getId())
                                           .result(MCPResponse.Result.of("nether 当前天气: "+getWeatherAndRainfall(Objects.requireNonNull(server.getLevel(Level.NETHER)))))
                                           .build();
                case "end" -> MCPMessage.responseBuilder()
                                 .id(request.getId())
                                 .result(MCPResponse.Result.of("end 当前天气: "+getWeatherAndRainfall(Objects.requireNonNull(server.getLevel(Level.END)))))
                                 .build();
