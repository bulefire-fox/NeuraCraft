...
import static com.bulefire.neuracraft.core.agent.commnd.util.ComponentGenerator.withHoverAndCopy;
        ...
        for (Agent agent : AgentController.getAgentManager().getAllAgents()) {
            var join = Component.literal("join in")
                                .withStyle(style -> style
                                        ...
                                        .withHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, Component.translatable("neuracraft.command.list.chatroom.hover.join", agent.getUUID().toString())))
                                        .withClickEvent(new ClickEvent(ClickEvent.Action.RUN_COMMAND, "/agent join " + agent.getUUID()))
                                );
            component.append(Component.translatable("neuracraft.command.list.chatroom.single",
                                                    withHoverAndCopy(agent.getName(), "#7CFC00"),
                                                    withHoverAndCopy(agent.getUUID().toString(), "#00ffff"),
                                                    join
            ));
        }
        feedback(commandContext.getSource(), Component.translatable("neuracraft.command.list.chatroom", component));
        return 1;
    }
}
